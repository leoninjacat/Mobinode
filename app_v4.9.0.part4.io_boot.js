...
"use strict";
console.log("%c[IO_BOOT] CARREGADO: v4.8.2-rc PDFFIX-TEST A", "color:#0f0;font-weight:bold");
...
function bindUI() {
    ...
    document.addEventListener("keydown", (ev) => {
        ...
        if (dom.stationQuickPick) {
            dom.stationQuickPick.addEventListener("change", () => {
                if (sidebarIsUpdating) return;

                const id = dom.stationQuickPick.value;
                if (!id) return;

                // troca seleção pra essa estação
                clearSelection();
...
function boot() {
    ...
    console.log("Mobinode: app.js carregou ✅ (v4.8.0)");
}
