...
function makeStationShapeEl(shape, cx, cy, r, wMul = 1, hMul = 1) {
 ...
 signageNameWithBadge: false,
	 ...
	 textNextColor: "",
	 textNextOutline: true,
	 signageNextSize: CFG.SIGNAGE_DEFAULT_SIZE,
 signageNextColor: "",
 signageNextOutline: true,
 ...
 accShapes: $("accShapes"),
 shapesEmpty: $("shapesEmpty"),
 shapesPanel: $("shapesPanel"),

 // shapes controls
 shapeKind: $("shapeKind"),
 btnShapeCreate: $("btnShapeCreate"),
 btnShapeDuplicate: $("btnShapeDuplicate"),

 shapeFillMode: $("shapeFillMode"),
 shapeFillColor: $("shapeFillColor"),
 shapeOpacity: $("shapeOpacity"),
 shapeOpacityVal: $("shapeOpacityVal"),

 shapeW: $("shapeW"),

 shapeWVal: $("shapeWVal"),
 shapeHVal: $("shapeHVal"),
 shapeRXVal: $("shapeRXVal"),

 shapeSides: $("shapeSides"),
 shapeSidesVal: $("shapeSidesVal"),

 // dynamic rows
 rowShapeW: $("rowShapeW"),
 rowShapeH: $("rowShapeH"),
 rowShapeRX: $("rowShapeRX"),
 rowShapeSides: $("rowShapeSides"),
 shapeWLabel: $("shapeWLabel"),
 ...
 shapeRX: $("shapeRX"),
  ...
  // arc controls
  arcControls: $("arcControls"),
  shapeArcR: $("shapeArcR"),
  shapeArcRVal: $("shapeArcRVal"),
  shapeArcT: $("shapeArcT"),
  shapeArcTVal: $("shapeArcTVal"),
  shapeArcA0: $("shapeArcA0"),
  shapeArcA0Val: $("shapeArcA0Val"),
  shapeArcA1: $("shapeArcA1"),
  shapeArcA1Val: $("shapeArcA1Val"),

 btnShapeApply: $("btnShapeApply"),
 btnShapeDelete: $("btnShapeDelete"),
...
function setSectionState(sectionEl, emptyEl, contentEl, enabled) {
 ...
 y: (clientY - r.top - state.view.y) / state.view.z,
    ...
    function updateCursor() {
        ...
        // v4.9.2: no modo "drag" (criar/conectar) o cursor vira crosshair
        if ((state.tool === "network" || state.tool === "line") && state.nodeDragMode === "drag") {
            dom.viewport.style.cursor = "crosshair";
            return;
        }
        ...
        if (state.tool === "shapes") {
            dom.viewport.style.cursor = state.shapeCreateArmed ? "crosshair" : "default";
            return;
        }
...
function renderLineEndpointBadges() {
    ...
    function ensureTextProps(t) {
        ...
        // Shapes (não são texto)
        if (t.kind === "shapeRect" || t.kind === "shapeCircle" || t.kind === "shapeArc" || t.kind === "shapePoly") {
            if (typeof t.fillMode !== "string") t.fillMode = "line";
            if (typeof t.lineId !== "string") t.lineId = state.activeLineId;
            if (typeof t.fill !== "string") t.fill = "#a020f0";
            if (!Number.isFinite(+t.opacity)) t.opacity = 1;

            if (!Number.isFinite(+t.w)) t.w = 640;
            if (!Number.isFinite(+t.h)) t.h = 140;
            ...
            if (t.kind === "shapePoly") {
                ...
                const sdi = Number.isFinite(sd) ? Math.round(sd) : 6;
                t.sides = clamp(sdi, 3, 24);
                // usa w/h como bounding box do polígono
                if (!Number.isFinite(+t.w)) t.w = 140;
                if (!Number.isFinite(+t.h)) t.h = 140;
                t.rx = 0;
                // r não é usado aqui
            }
            ...
            // Arc (donut recortado)
            if (t.kind === "shapeArc") {
                if (!Number.isFinite(+t.rOuter)) t.rOuter = 80;
                if (!Number.isFinite(+t.thickness)) t.thickness = 30;
                if (!Number.isFinite(+t.a0)) t.a0 = -45;
                if (!Number.isFinite(+t.a1)) t.a1 = 225;
            }
            return; // importantíssimo: não cai na lógica de texto
        }
        ...
        if (typeof t.outline !== "boolean") t.outline = true;
    ...
    function applyTextRotation(value) {
 ...
 selectedTextId: state.selectedTextId,
    ...
    function normDeltaDeg(a0, a1) {
        // delta em [0, 360)
        let d = (a1 - a0) % 360;
        if (d < 0) d += 360;
        return d;
    }
    function arcDonutPath(cx, cy, rOuter, thickness, a0, a1) {
        const ro = Math.max(1, +rOuter || 1);
        const th = Math.max(1, +thickness || 1);
        const ri = Math.max(1, ro - th);
        const p0o = polar(cx, cy, ro, a0);
        const p1o = polar(cx, cy, ro, a1);
        const p1i = polar(cx, cy, ri, a1);
        const p0i = polar(cx, cy, ri, a0);
        const delta = normDeltaDeg(a0, a1);
        const large = (delta > 180) ? 1 : 0;
        const sweep = 1;
        return [
            `M ${p0o.x} ${p0o.y}`,
            `A ${ro} ${ro} 0 ${large} ${sweep} ${p1o.x} ${p1o.y}`,
            `L ${p1i.x} ${p1i.y}`,
            `A ${ri} ${ri} 0 ${large} ${1 - sweep} ${p0i.x} ${p0i.y}`,
            `Z`,
        ].join(" ");
    }

    function renderTexts() {
        const applyTextOutline = (el, selected, outlineOn) => {
            if (!el) return;
            if (selected) {
                el.setAttribute("stroke", "rgba(255,215,120,.92)");
        ...
        for (const tdata of state.texts) {
            ...
            // Shapes (retângulo / círculo / arco / polígono)
            // Ctrl+F: Shapes (retângulo / círculo / arco / polígono)
            if (kind === "shapeRect" || kind === "shapeCircle" || kind === "shapeArc" || kind === "shapePoly") {
                const g = el("g");
                g.setAttribute("class", `map-text map-shape${selected ? " selected" : ""}`);
                g.setAttribute("data-id", tdata.id);
                ...
                if (kind === "shapeRect") {
                    ...
                    c.setAttribute("fill-opacity", String(tdata.opacity ?? 1));
                    g.appendChild(c);
                } else if (kind === "shapePoly") {
                    const sides = Math.max(3, Math.min(24, Math.round(+tdata.sides || 6)));
                    const rx = Math.max(2, (Number.isFinite(+tdata.w) ? (+tdata.w / 2) : 70));
                    const ry = Math.max(2, (Number.isFinite(+tdata.h) ? (+tdata.h / 2) : 70));
                    const step = 360 / sides;

                    const pts = [];
                    for (let i = 0; i < sides; i++) {
                        const a = deg2rad(-90 + i * step);
                    ...
                    const p = el("path");
                    p.setAttribute("class", "shape-body");
                    p.setAttribute("d", d);
                    p.setAttribute("fill", fill);
                    p.setAttribute("fill-opacity", String(tdata.opacity ?? 1));
                    g.appendChild(p);
                } else {
                    // shapeArc: donut recortado
                    const p = el("path");
                    p.setAttribute("class", "shape-body");
                    const d = arcDonutPath(0, 0, tdata.rOuter || 80, tdata.thickness || 30, tdata.a0 ?? -45, tdata.a1 ?? 225);
                    p.setAttribute("d", d);
                    p.setAttribute("fill", fill);
                    p.setAttribute("fill-opacity", String(tdata.opacity ?? 1));
                    g.appendChild(p);
            ...
            if (kind === "badgeName") {
                ...
                nt.style.setProperty('--map-text-fill', fill);
                applyTextOutline(nt, selected, !!tdata.outline);
                nt.textContent = name;
                g.appendChild(nt);
            ...
            if (tdata.rotation) {
                t.setAttribute(
                    "transform",
                    `rotate(${tdata.rotation}, ${tdata.x}, ${tdata.y})`
                );
            }
            ...
            // Cor do texto: se tiver cor custom, usa.
            // Senão: nome de linha herda a cor da linha; texto livre usa a cor padrão do tema via CSS var.
            if (tdata.color) {
                t.style.setProperty('--map-text-fill', String(tdata.color));
            } else if (kind === "name") {
                const fill = line?.color || tdata.fallbackColor || "rgba(255,255,255,.92)";
                t.style.setProperty('--map-text-fill', fill);
            }
...
function deleteEdge(edgeId) {
    ...
    function addText(x, y, opts = null) {
        ...
        if (opts && (opts.kind === "badge" || opts.kind === "name" || opts.kind === "badgeName")) {
            ...
            const t = {
                ...
                color: (opts && typeof opts.color === "string") ? opts.color : (state.signageNextColor || ""),
                outline: (opts && typeof opts.outline === "boolean") ? opts.outline : !!state.signageNextOutline,
                rotation: (typeof state.textNextRotation === "number" ? state.textNextRotation : 0),
        ...
        const t = { id, x: p.x, y: p.y, kind: "text", text: "Texto", size: CFG.TEXT_DEFAULT_SIZE, bold: true, italic: false,
            color: (opts && typeof opts.color === "string") ? opts.color : (state.textNextColor || ""),
            outline: (opts && typeof opts.outline === "boolean") ? opts.outline : !!state.textNextOutline
        };
        ensureTextProps(t);
        state.texts.push(t);
        return t;
    ...
    function addShape(x, y, opts = null) {
        const p = worldToGrid({ x, y });
        const id = uid();

        const kind = (opts && (opts.kind === "shapeRect" || opts.kind === "shapeCircle" || opts.kind === "shapeArc" || opts.kind === "shapePoly")) ? opts.kind : "shapeRect";
        ...
        const t = {
            ...
            // style
            fillMode: (opts && opts.fillMode) ? opts.fillMode : "line",  // "line" | "custom"
            lineId: (opts && typeof opts.lineId === "string") ? opts.lineId : state.activeLineId,
            fill: (opts && typeof opts.fill === "string") ? opts.fill : "#a020f0",
            opacity: (opts && Number.isFinite(+opts.opacity)) ? +opts.opacity : 1,

            // geometry
            w: (opts && Number.isFinite(+opts.w)) ? Math.max(10, +opts.w) : 640,
            h: (opts && Number.isFinite(+opts.h)) ? Math.max(10, +opts.h) : 140,
            rx: (opts && Number.isFinite(+opts.rx)) ? Math.max(0, +opts.rx) : 18,
            sides: (opts && Number.isFinite(+opts.sides)) ? Math.round(+opts.sides) : 6,
            r: (opts && Number.isFinite(+opts.r)) ? Math.max(2, +opts.r) : 40,
            // arc geometry (donut)
            rOuter: (opts && Number.isFinite(+opts.rOuter)) ? Math.max(5, +opts.rOuter) : 80,
            thickness: (opts && Number.isFinite(+opts.thickness)) ? Math.max(1, +opts.thickness) : 30,
            a0: (opts && Number.isFinite(+opts.a0)) ? +opts.a0 : -45,
            a1: (opts && Number.isFinite(+opts.a1)) ? +opts.a1 : 225,
        };
...
function nearestNode(worldPt, radius = CFG.CONNECT_SNAP_RADIUS) {
    ...
    function setTool(tool) {
        ...
        // v4.9.3: no mobile, a ajuda rápida só aparece quando o usuário clicar no botão (❓).
        // No desktop, mantém o comportamento de auto-abrir ao trocar de ferramenta.
        try {
            const isMobile = !!(window.matchMedia && window.matchMedia("(max-width: 820px)").matches);
            if (!isMobile && dom.helpPanel && dom.helpPanel.style.display === "none") showHelpPanel(true);
        } catch {}
